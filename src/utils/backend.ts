...
export const privyGetAllUsers = async (
    ...
    let cursor
    let query
    const snakeCaseUsers: PrivyUser[] = []
    ...
    do {
        ...
        const newUsers = query.data
        console.log('newUsers length', newUsers.length)
        snakeCaseUsers.push(...newUsers)
        cursor = query.next_cursor
    } while (cursor !== null)

    console.log('users:', snakeCaseUsers.length)

    const users = snakeCaseUsers.map((user: any) => {
